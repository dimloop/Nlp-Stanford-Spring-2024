...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    theta_i =  1.0 / (10000 ** (-2 * (torch.arange(dim/2) / dim)))
    t = torch.arange(max_positions).float().view(-1, 1)
    angle = t * theta_i.unsqueeze(0) 
    cos_vals = torch.cos(angle)  # Shape: (max_positions, dim/2)
    sin_vals = torch.sin(angle)  # Shape: (max_positions, dim/2)
...
def apply_rotary_emb(x, rope_cache):
    ...
    # Convert x to complex numbers (reshape pairs of values)
    x_complex = torch.view_as_complex(x.view(batch_size, seq_len, num_heads, dim_head // 2, 2))

    rope_emb = rope_cache[:seq_len]  # Shape: (seq_len, head_dim/2, 2)
    rope_emb = rope_emb.unsqueeze(1)

    # Convert RoPE embedding to complex form
    rope_complex = torch.view_as_complex(rope_emb)

    # Apply rotation (complex multiplication)
    rotated_x_complex = x_complex * rope_complex  # Element-wise rotation

    # Convert back to real-valued tensor
    rotated_x = torch.view_as_real(rotated_x_complex).reshape(batch_size, seq_len, num_heads, dim_head)
    ### END YOUR CODE ###
    return rotated_x
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.rope:
            ...
            v = apply_rotary_emb(v, self.rope_cache)
            ### END YOUR CODE ###
